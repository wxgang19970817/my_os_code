/*************************************************************************
	...
	> Created Time: 2021年12月30日 星期四 15时11分21秒
 ************************************************************************/
...
{
	uint32_t fd_idx = 3; /* 文件表的前三个预留给标准输入、输出、错误 */
...
/* 将全局描述符下标安装到进程或线程自己的文件描述符数组 fa_table中，成功返回下标，失败返回-1 */
...
void bitmap_sync(struct partition *part, uint32_t bit_idx, uint8_t btmp_type)
{
	...
	/* 目前文件系统中的位图只有inode_bitmap和block_bitmap */
	switch (btmp_type)
	{
	case INODE_BITMAP:
...
int32_t file_create(struct dir *parent_dir, char *filename, uint8_t flag)
{
	...
	/*创建文件的顺序：创建文件i结点->文件描述符fd->目录项*/

	/* 创建文件i结点：分配inode号->申请结点内存 */
	/* 为新文件分配inode */
	...
	/* 创建文件描述符fd:从文件表中找到空闲的文件结构->为空闲文件结构赋值 */
	/* 返回的是file_table数组中空闲文件结构的下标 */
	...
	memset(&new_dir_entry, 0, sizeof(struct dir_entry)); /* 将目录项清0 */
	/* 填充刚才创建的目录项 */
	create_dir_entry(filename, inode_no, FT_REGULAR, &new_dir_entry); /* create_dir_entry只是内存操作不出意外，不会返回失败 */

	/* 同步内存数据到硬盘 */
	/* a 在目录parent_dir下安装目录项new_dir_entry,写入硬盘后返回true,写入硬盘后返回true,否则false */
	...
	/* b 将刚才改变的父目录inode的内容同步到硬盘 */
	inode_sync(cur_part, parent_dir->inode, io_buf); /* 由主调者提供io_buf */
...
rollback:
	...
	{
	/* 每个case没有break，是一种累加回滚，case 3需要回滚的最多 */
	case 3:
...
int32_t file_open(uint32_t inode_no, uint8_t flag)
{
	...
	bool *write_deny = &file_table[fd_idx].fd_inode->write_deny; /* 写文件不能并行的标识 */
...
int32_t file_write(struct file *file, const void *buf, uint32_t count)
{
	...
	uint8_t *io_buf = sys_malloc(BLOCK_SIZE); /* 申请512字节当缓冲区 */
	if (io_buf == NULL)
	{
	...
	uint32_t *all_blocks = (uint32_t *)sys_malloc(BLOCK_SIZE + 48); /* 用来记录文件所有的块地址,统一管理比较方便  (12 + 128)*4 */
	if (all_blocks == NULL)
	{
	...
	uint32_t sec_left_bytes;	   /* 扇区内字节剩余量 */
	...
	if (add_blocks == 0)
	{
		...
		{
			/* 未写入新数据之前已经占用了间接块，需要将间接块的地址读进来 */
			ASSERT(file->fd_inode->i_sectors[12] != 0);
			indirect_block_table = file->fd_inode->i_sectors[12]; /* 一级间接表的地址 */
			ide_read(cur_part->my_disk, indirect_block_table, all_blocks + 12, 1);
		}
	}
	else
	{
		/* 如果有增量，便涉及到分配新扇区及是否分配一级间接表块，下面要分三种情况处理 */
		/* 第一种情况：12个直接块够用 */
		if (file_will_use_blocks <= 12)
		{
			...
			while (block_idx < file_will_use_blocks)
			{
				...
				/* 写文件时，不应该存在块未使用但是已经分配扇区的情况，当文件删除时，就会把块地址清0 */
				ASSERT(file->fd_inode->i_sectors[block_idx] == 0);
...
int32_t file_read(struct file *file, void *buf, uint32_t count)
{
	...
	uint8_t *io_buf = sys_malloc(BLOCK_SIZE); /* 还是申请缓冲区，将来会从硬盘中读出的数据存入io_buf */
	if (io_buf == NULL)
	{
	...
	uint32_t read_blocks = block_read_start_idx - block_read_end_idx; /* 增量为0，表示数据在同一扇区 */
	ASSERT(block_read_start_idx < 139 && block_read_end_idx < 139);

	int32_t indirect_block_table; /* 用力爱获取一级间接表地址 */
