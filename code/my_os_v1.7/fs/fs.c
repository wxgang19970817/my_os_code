/*************************************************************************
	...
	> Created Time: 2021年12月27日 星期一 21时01分15秒
 ************************************************************************/
...
static void partition_format(struct partition *part)
{
	...
	uint32_t inode_bitmap_sects = DIV_ROUND_UP(MAX_FILES_PER_PART, BITS_PER_SECTOR); /* i结点位图占用的扇区数，最多支持4096个 */
	/* inode数组占用的扇区大小，取决于inode的尺寸和数量 */
	...
	block_bitmap_sects = DIV_ROUND_UP(free_sects, BITS_PER_SECTOR); /* 空闲块占用的扇区数 */

	/* block_bitmap_bit_len是位图中位的长度，也是可用块的数量 */
	uint32_t block_bitmap_bit_len = free_sects - block_bitmap_sects;
	block_bitmap_sects = DIV_ROUND_UP(block_bitmap_bit_len, BITS_PER_SECTOR); /* 空闲块位图最终占用的扇区数 */
	/********************************************************************************/

	/* 超级块初始化 */
	/* 这里用的是局部变量，所以使用的是栈中的内存 */
	...
	sb.block_bitmap_lba = sb.part_lba_base + 2; /* 第0块是引导块，第1块是超级块,再往后就是块位图了 */
	...
	/* 一、 将超级块写入本分区的1扇区 */
	ide_write(hd, part->start_lba + 1, &sb, 1); /* 跨过引导扇区OBR,把超级块写入后面的扇区 */
	printk(" super_block_lba:0x%x\n", part->start_lba + 1);

	/* 找出数据量最大的元信息，用其尺寸做存储缓冲区,超级块勉强用栈可以，但是空闲块位图和inode数组位图占用扇区数太大，需要用堆 */
	uint32_t buf_size = (sb.block_bitmap_sects >= sb.inode_bitmap_sects ? sb.block_bitmap_sects : sb.inode_bitmap_sects);
	buf_size = (buf_size >= sb.inode_table_sects ? buf_size : sb.inode_table_sects) * SECTOR_SIZE;
	uint8_t *buf = (uint8_t *)sys_malloc(buf_size); /* 申请的内存由内存管理系统清0后返回 */
	/* 二、 将块位图初始化并写入sb.block_bitmap_lba */
	...
	/* 1 先将位图最后一字节到其所在的扇区的结束置为1，即超出实际块数的部分直接置为已占用 */
	...
	/* 三、将inode位图初始化并写入sb.inode_bitmap_lba */
	/* 先清空缓冲区 */
	memset(buf, 0, buf_size);
	buf[0] |= 0x1; /* 第0个inode分给了根目录 */
	/* 由于inode_table中共4096个inode,位图inode_bitmap正好占用1扇区，即inode_bitmap_sects等于1
		所以位图中的位全都代表inode_table中的inode,无须再像block_bitmap那样单独处理最后一扇区的剩余部分
		inode_bitmap所在的扇区中没有多余的无效位 */
	ide_write(hd, sb.inode_bitmap_lba, buf, sb.inode_bitmap_sects);

	/* 四、将inode数组初始化并写入sb.inode_table_lba */
...
char* path_parse(char *pathname, char *name_store)
{
	if (pathname[0] == '/')
	{
		/* 根目录不需要单独解析，路径汇总出现1个或多个连续的字符'/',将这些'/'跳过，如"///a/b"" */
		while (*(++pathname) == '/');
	}
...
static int search_file(const char *pathname, struct path_search_record *searched_record)
{
	...
	{
		/* 若第一个字符就是结束符，结束循环 */
		/* 记录查找过的路径，但不能超过searched_path 的长度512字节 */
		ASSERT(strlen(searched_record->searched_path) < 512);

		/* 记录已存在的父目录 */
		strcat(searched_record->searched_path, "/"); /* 第一次执行的时候是根目录/,后边循环是路径分隔符 */
		...
		{
			/* 若找不到，则返回-1,找不到目录项时，要留着parent_dir不要关闭，若是创建新文件的话需要在parent_dir中创建 */
			return -1;
		}
	}
...
/* open函数不仅能打开文件还能创建新文件 */
int32_t sys_open(const char *pathname, uint8_t flags)
{
	/* 对目录要用dir_open,这里只有open文件,以'/'字符结尾的便是目录*/
	...
	/* 生成路径搜索记录变量，用来记录文件查找时所遍历过得目录，会作为参数传给函数search_file */
	struct path_search_record searched_record;
	/* searched_record位于栈中，栈中的数据不会自动清0 */
	...
	case O_CREAT:
		...
		break;
	default:
		/* 其余为打开已存在文件,O_RDONLY,O_WRONLY,O_RDWR */
...
uint32_t fd_local2global(uint32_t local_fd)
{
	...
	int32_t global_fd = cur->fd_table[local_fd]; /* 文件描述符是数组下标，数组的内容是文件表的下标 */
...
int32_t sys_write(int32_t fd, const void *buf, uint32_t count)
{
	...
	{
		uint32_t _fd = fd_local2global(fd); /* 将文件描述符转化为文件表的下标 */
...
int32_t sys_lseek(int32_t fd,int32_t offset,uint8_t whence)
{
	...
	switch(whence)
	{
		...
		/* SEEK_CUR    新的读写位置是相对于当前位置增加offset个位移量 */
		case SEEK_CUR:		/* offset可正可负 */
			new_pos = (int32_t)pf->fd_pos + offset;
			break;
		
		/* SEEK_END　新的读写位置是相对于文件尺寸再增offset个位移量 */
		...
		while (dev_no < 2)
		{
			...
			while (part_idx < 12)
			{
				...
				/* channels数组是全局变量，默认值为0,disk属于其嵌套结构
					partition 又为disk的嵌套结构，因此partition中的成员默认也为0
