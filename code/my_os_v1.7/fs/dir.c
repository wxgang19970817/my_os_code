/*************************************************************************
	...
	> Created Time: 2021年12月30日 星期四 21时12分02秒
 ************************************************************************/
...
bool search_dir_entry(struct partition* part,struct dir* pdir,const char* name,struct dir_entry* dir_e)
{
	...
	uint32_t* all_blocks = (uint32_t*)sys_malloc(48 + 512);			/* 140个块的地址，一个地址4字节,总共560 */
	if(all_blocks == NULL)
	{
	...
	/* 写目录项的时候已保证目录项不跨区，这样读目录项时，容易处理，只申请容纳1个扇区的内存 */
	...
	/* 由于我们不知道目录项在目录的哪个扇区中存在，开始在所有块中查找目录项 */
	while(block_idx < block_cnt)
	{
		/* 快地址为0时表示该块中无数据，继续在其它块中找 */
		if(all_blocks[block_idx] == 0)
		{
			block_idx++;
			continue;
		}
		/* 从扇区地址all_blocks读入1扇区数据到buf */
		...
		memset(buf,0,SECTOR_SIZE);			/* 将buf清0,下次使用 */
	}
	sys_free(buf);
	sys_free(all_blocks);
	return false;
...
bool sync_dir_entry(struct dir* parent_dir,struct dir_entry* p_de,void* io_buf)
{
	...
	uint32_t dir_size = dir_inode->i_size;		/* 当inode是目录项时，i_size是目录中目录项的大小之和 */
	uint32_t dir_entry_size = cur_part->sb->dir_entry_size;

	ASSERT(dir_size % dir_entry_size == 0);			/* dir_size 应该是dir_entry_size的整数倍 */

	uint32_t dir_entrys_per_sec = (512 / dir_entry_size);			/* 每个扇区可完整容纳的最大目录项数目 */
	...
	　因为在删除文件的时候会造成目录中的空洞，要逐个目录项查找空位，避免一味在目录的末尾添加目录项 */
	block_idx = 0;
	while(block_idx < 140)
	{
		...
		if(all_blocks[block_idx] == 0)
		{
			/* if里判断没有分配块，这里分配一个扇区并返回地址(此时操作的是内存的块位图) */
			...
			/* 为了保持数据同步，将内存中的数据同步到硬盘，每分配一个块就同步一次block_bitmap */
			block_bitmap_idx = block_lba - cur_part->sb->data_start_lba;	/* 计算出分配的扇区的偏移量 */
	...
	while(block_idx < 140)
	{
		...
		if(dir_entry_cnt == 1 && !is_dir_first_block)
		{
			...
			else
			{
				...
				{
					/* 间接索引表中就当前这1个间接块，直接把间接索引表所在的块回收，然后查出间接索引表块地址 */
					/* 回收间接索引表所在的块 */
...
{
	/* 这个dir_buf就是用来存放目录项的，这里dir_e指向这个buf */
	...
	while(block_idx < block_cnt)
	{
		...
		while(dir_entry_idx < dir_entrys_per_sec)
		{
			...
			{
				/* 如果f_type不等于0，即不等于FT_UNKNOWN,判断是不是最新的目录项，避免返回曾经已经返回过的目录项 */
...
int32_t dir_remove(struct dir* parent_dir,struct dir* child_dir)
{
	...
	/* 在父目录中parent_dir中删除子目录child_dir对应的目录 */
