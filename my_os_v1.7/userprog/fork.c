/*************************************************************************
	...
	> Created Time: 2022年01月05日 星期三 16时15分48秒
 ************************************************************************/
...
{
	/* a  复制pcb所在的整个页，里面包含进程pcb信息及特级0栈，里面包含了返回地址，然后再单独修改个别部分 */
	...
	block_desc_init(child_thread->u_block_desc);	/* 初始化子进程的内存块描述符，如果不初始化的话用的是父进程的块描述符，会报缺页异常 */

	/* b  复制父进程的虚拟地址池的位图 */
	uint32_t bitmap_pg_cnt = DIV_ROUND_UP((0xc0000000 - USER_VADDR_START) / PG_SIZE / 8,PG_SIZE);
	void* vaddr_btmp = get_kernel_pages(bitmap_pg_cnt);		/* 申请页框来存储位图 */
	if(vaddr_btmp == NULL)
	{
		return -1;
	}	
...
/* 复制子进程的进程体(代码和数据)及用户栈,buf_page必须是内核页 */
static void copy_body_stack3(struct task_struct* child_thread,struct task_struct* parent_thread,void* buf_page)
{
	...
	/* 在父进程的用户空间中查找已有数据的页,只要把用户空间有效的有数据部分拷贝出来就行 */
	while(idx_byte < btmp_bytes_len)
	{
		/* 虚拟地址位图管理了包括用户进程体、堆、栈占用的内存,要遍历位图的每一位 */
		if(vaddr_btmp[idx_byte])
		{
			...
			while( idx_bit < 8)
			{
				if((BITMAP_MASK << idx_bit) & vaddr_btmp[idx_byte])
				{
					...
					/* 我们的目的是将父进程用户控件的数据复制到子进程的用户空间，而每个进程的低3G空间是独立的，
						但是内核1G空间是共有的，所以要用内核空间作为中转 */
...
static int32_t build_child_stack(struct task_struct* child_thread)
{
	...
	/* fork要为子进程返回0值，根据ABI的约定，eax寄存器中是函数的返回值，所以这里将eax置0 */
	intr_0_stack->eax = 0;

	/* b 为switch_to构建 struct thread_stack,将其构建在紧临intr_stack之下的空间 */
	uint32_t* ret_addr_in_thread_stack = (uint32_t*)intr_0_stack - 1;

	/* 下面这三行不是必要的，只是为了梳理thread_stack的关系 */
	uint32_t* esi_ptr_in_thread_stack = (uint32_t*)intr_0_stack - 2;
	uint32_t* edi_ptr_in_thread_stack = (uint32_t*)intr_0_stack - 3;
	uint32_t* ebx_ptr_in_thread_stack = (uint32_t*)intr_0_stack - 4;
	/***************************************************************/

	/* ebp在thread_stack中的地址便是当时的esp(0级栈的栈顶),即esp为"(uint32*)intr_0_stack - 5" */
	uint32_t* ebp_ptr_in_thread_stack = (uint32_t*)intr_0_stack - 5;

	/* switch_to的返回地址更新为intr_exit,直接从中断返回 */
	*ret_addr_in_thread_stack = (uint32_t)intr_exit;

	/* 下面这两行赋值只是为了使构建的thread_stack更加清晰，其实也不需要，因为在进入intr_exit后一系列的pop会把寄存器中的数据覆盖 */
