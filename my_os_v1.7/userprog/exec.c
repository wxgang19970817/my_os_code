/*************************************************************************
	...
	> Created Time: 2022年01月07日 星期五 13时48分31秒
 ************************************************************************/
...
static bool segment_load(int32_t fd,uint32_t offset,uint32_t filesz,uint32_t vaddr)
{
	...
	while(page_idx < occupy_pages)
	{
		...
		}		/* 如果原进程的页表已经分配好了，利用现有的物理页，直接覆盖进程体 */
		vaddr_page	+=	PG_SIZE;
		page_idx++;
	}
...
static int32_t load(const char* pathname)
{
	...
	{
		/* e_ident[16]数组包含ELF魔数、32位/64位、大小端字节序、elf版本信息等
			e_type = 2 表示ET_EXEC 可执行文件
			e_machine = 3 表示在EM_386平台使用 
			e_version  = 1 表示版本信息，e_phnum 段的数量，e_phentsize 指描述段信息的数据结构的字节大小*/
		ret = -1;
		goto done;
	}
...
int32_t	sys_execv(const char* path,const char* argv[])
{
	...
	int32_t entry_point = load(path);			/* 加载进程成功之后要修改内核栈中的参数 */
	if(entry_point == -1)
	{
		/* 若失败则返回-1 */
		return -1;
