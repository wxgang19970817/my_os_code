/*************************************************************************
	> File Name: wait_exit.c
	> Author: wxgang
	> Mail: wxgang19970817@163.com 
	> Created Time: 2022年01月08日 星期六 15时06分34秒
 ************************************************************************/
...
static bool find_child(struct list_elem* pelem,int32_t ppid)
{
	...
	if(pthread->parent_pid == ppid)
	{
		...
		return true;		/* list_traversal 只有在回调函数返回true时才会停止继续遍历，所以在此返回true */
	}
...
/* list_traversal的回调函数
	将一个子进程过继给init */
static void init_adopt_a_child(struct list_elem* pelem,int32_t pid)
{
	struct task_struct* pthread = elem2entry(struct task_struct,all_list_tag,pelem);
